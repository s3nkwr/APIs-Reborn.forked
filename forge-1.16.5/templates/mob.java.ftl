...
@${JavaModName}Elements.ModElement.Tag public class ${name}Entity extends ${JavaModName}Elements.ModElement {
	...
	public static class CustomEntity extends ${extendsClass}Entity<#if data.ranged && !settings.getMCreatorDependencies().contains("geckolib")> implements IRangedAttackMob</#if><#if data.ranged && settings.getMCreatorDependencies().contains("geckolib")> implements IRangedAttackMob, IAnimatedEntity</#if><#if !data.ranged && settings.getMCreatorDependencies().contains("geckolib")> implements IAnimatedEntity</#if> {
	...
	<#if data.getModelCode()?? && !data.isBuiltInModel() >
		<#if settings.getMCreatorDependencies().contains("geckolib")>
			${data.getModelCode().toString()
				...
				.replace("MODID", "${modid}")
				.replace("ANIMATIONFILE", "${registryname}")
			}
		<#else>
			${data.getModelCode().toString()
				.replace("extends ModelBase", "extends EntityModel<Entity>")
